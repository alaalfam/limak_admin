...
final class Projects_Controller extends WP_REST_Controller implements Registrable {
	...
	public function get_items( $request ) {
		...
		$items = array_map( [ Project_Transformer::class, 'to_summary' ], $query->posts );
	...
	public function get_item( $request ) {
		...
		if ( ! $post ) {
			return new WP_Error(
				'limak_project_not_found',
				__( 'Project not found.', 'limak-headless' ),
				[ 'status' => 404 ]
			);
		}

		return new WP_REST_Response( Project_Transformer::to_detail( $post ) );
	}
